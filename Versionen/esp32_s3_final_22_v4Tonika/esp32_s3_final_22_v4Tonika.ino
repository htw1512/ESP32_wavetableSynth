// ESP32-S3 - Polyphoner Synth - (V34.5.1 - ArpTonicMode Fixes)
...
void generateAllSynthWavetables() { 
    ...
    if (current_wave_gen_idx < NUM_WAVETABLES_TOTAL) { for(int i=0;i<WAVETABLE_SIZE;++i){float t=2*PI*(float)i/WAVETABLE_SIZE; wavetables[current_wave_gen_idx][i]=sinf(t)+(1.0f/3.0f)*sinf(3*t)+(1.0f/5.0f)*sinf(5*t);} normalizeWavetable(current_wave_gen_idx); wavetable_display_names[current_wave_gen_idx++] = "HollowSquare"; }
    if (current_wave_gen_idx < NUM_WAVETABLES_TOTAL) { for(int i=0;i<WAVETABLE_SIZE;++i){float t=(float)i/WAVETABLE_SIZE; wavetables[current_wave_gen_idx][i]=0.6f*sinf(2*PI*t)+0.4f*sinf(2*PI*t*1.8f)+0.1f*sinf(2*PI*t*4.5f);} normalizeWavetable(current_wave_gen_idx); wavetable_display_names[current_wave_gen_idx++] = "Formant O"; }
    // Zusätzliche 30 Wellenformen
    ...
    if (current_wave_gen_idx < NUM_WAVETABLES_TOTAL) { for (int k = 1; k <= 9; k += 2) for (int i = 0; i < WAVETABLE_SIZE; ++i) wavetables[current_wave_gen_idx][i] += (1.0f / (k*k)) * sinf(2.0f * PI * k * (float)i / WAVETABLE_SIZE); normalizeWavetable(current_wave_gen_idx); wavetable_display_names[current_wave_gen_idx++] = "Hollow Sq X"; }
    ...
    // 5 zusätzliche, weniger obertonreiche Wellenformen
    if (current_wave_gen_idx < NUM_WAVETABLES_TOTAL) { for (int i = 0; i < WAVETABLE_SIZE; ++i) { wavetables[current_wave_gen_idx][i] = 0.8f * sinf(2.0f*PI*(float)i/WAVETABLE_SIZE) + 0.2f * sinf(4.0f*PI*(float)i/WAVETABLE_SIZE); } normalizeWavetable(current_wave_gen_idx); wavetable_display_names[current_wave_gen_idx++] = "Soft Sine"; }
    if (current_wave_gen_idx < NUM_WAVETABLES_TOTAL) { for (int k = 1; k <= 5; k += 2) for (int i = 0; i < WAVETABLE_SIZE; ++i) wavetables[current_wave_gen_idx][i] += (1.0f / k) * sinf(2.0f * PI * k * (float)i / WAVETABLE_SIZE); normalizeWavetable(current_wave_gen_idx); wavetable_display_names[current_wave_gen_idx++] = "Rounded Sq"; }
    if (current_wave_gen_idx < NUM_WAVETABLES_TOTAL) { for (int i = 0; i < WAVETABLE_SIZE; ++i) { float p = 2.0f * PI * (float)i / WAVETABLE_SIZE; wavetables[current_wave_gen_idx][i] = 0.7f * sinf(p) + 0.3f * sinf(p * 1.2599f); } normalizeWavetable(current_wave_gen_idx); wavetable_display_names[current_wave_gen_idx++] = "Sine +Maj3rd"; }
    if (current_wave_gen_idx < NUM_WAVETABLES_TOTAL) { float detune_factor = 1.015f; for (int i = 0; i < WAVETABLE_SIZE; ++i) { wavetables[current_wave_gen_idx][i] = 0.5f * sinf(2.0f * PI * (float)i / WAVETABLE_SIZE) + 0.5f * sinf(2.0f * PI * (float)i * detune_factor / WAVETABLE_SIZE); } normalizeWavetable(current_wave_gen_idx); wavetable_display_names[current_wave_gen_idx++] = "Soft Chorus"; }
    if (current_wave_gen_idx < NUM_WAVETABLES_TOTAL) { for(int i=0;i<WAVETABLE_SIZE;++i){float t=2*PI*(float)i/WAVETABLE_SIZE; wavetables[current_wave_gen_idx][i]=powf(sinf(t),5);} normalizeWavetable(current_wave_gen_idx); wavetable_display_names[current_wave_gen_idx++] = "Sine Quintic"; }
    
    Serial.printf("Total %d of %d wavetables generated.\n", current_wave_gen_idx, NUM_WAVETABLES_TOTAL);
    for (int i = current_wave_gen_idx; i < NUM_WAVETABLES_TOTAL; ++i) { 
        wavetable_display_names[i] = "Empty";
    }
}
...
void processArpControls() { 
    if (digitalRead(ARP_ONOFF_SWITCH_PIN) == LOW) { 
        if (millis() - last_arp_onoff_press_time > ARP_ONOFF_DEBOUNCE_TIME) { 
            arp_is_on = !arp_is_on; 
            if (!arp_is_on) { // Wenn Arp ausgeschaltet wird
                if (arp_last_played_arp_note != 0) { 
                    arpInternalNoteOff(arp_last_played_arp_note);
...
void updateDisplay() {
    ...
    if (global_osc_wave_select_idx < NUM_WAVETABLES_TOTAL) { 
        display.print(wavetable_display_names[global_osc_wave_select_idx]); 
...
void handleNoteOff(byte channel, byte note, byte velocity) {
    ...
    if (found_and_removed_sorted || found_and_removed_order) { 
        if (!arp_is_on) {
            ...
            uint8_t note_to_check_off = note;
            if (is_octave_mode && arp_octave_shift != 0) {
                // Wenn eine oktavierte Note losgelassen wird, ist die 'note' Variable die Basisnote.
                // Wir müssen prüfen, ob die gespielte Arp-Note (die oktaviert sein kann) der oktavierten 'note' entspricht.
                // ODER wenn die losgelassene Note die Basis der oktavierten gespielten Note ist.
                if (arp_last_played_arp_note != 0 && ( (note + arp_octave_shift * 12) == arp_last_played_arp_note ) ) {
                     // Die tatsächlich gespielte Arp-Note war die oktavierte Version der losgelassenen Note.
                     note_to_check_off = arp_last_played_arp_note; // Stoppe die tatsächlich gespielte Note
                }
            }


            if (arp_last_played_arp_note == note_to_check_off) {
                arpInternalNoteOff(arp_last_played_arp_note);
                arp_last_played_arp_note = 0;
            }

            if (is_tonic_mode && arp_last_played_tonic_note == note) { // Tonika ist immer Basisnote
                arpInternalNoteOff(arp_last_played_tonic_note);
                arp_last_played_tonic_note = 0;
            }
            
            if (arp_num_held_notes == 0) { 
                if (arp_last_played_arp_note != 0) {arpInternalNoteOff(arp_last_played_arp_note); arp_last_played_arp_note = 0;}
...
void processArpeggiator() {
    ...
    if (millis() - last_arp_step_time >= current_arp_interval_ms) {
        ...
        // Die Tonika wird nur gestoppt, wenn sie sich ändert oder der Arp stoppt (siehe unten)
        ...
        if (arp_num_held_notes > 0) {
            ...
            bool play_tonic_separately = false;

            if (current_arp_mode == ARP_UP_TONIC) { effective_mode = ARP_UP; play_tonic_separately = true; }
            else if (current_arp_mode == ARP_DOWN_TONIC) { effective_mode = ARP_DOWN; play_tonic_separately = true; }
            else if (current_arp_mode == ARP_UP_DOWN_TONIC) { effective_mode = ARP_UP_DOWN; play_tonic_separately = true; }
            else if (current_arp_mode == ARP_RANDOM_TONIC) { effective_mode = ARP_RANDOM; play_tonic_separately = true; }
            ...
            switch (effective_mode) {
                case ARP_UP: temp_arp_index = (temp_arp_index + 1) % arp_num_held_notes; base_note_for_arp_pattern = arp_held_notes[temp_arp_index]; break;
                case ARP_DOWN: temp_arp_index = temp_arp_index - 1; if (temp_arp_index < 0) { temp_arp_index = arp_num_held_notes - 1; } base_note_for_arp_pattern = arp_held_notes[temp_arp_index]; break;
                case ARP_UP_DOWN: if (arp_up_down_direction_is_up) { temp_arp_index = temp_arp_index + 1; if (temp_arp_index >= arp_num_held_notes) { temp_arp_index = arp_num_held_notes > 1 ? arp_num_held_notes - 2 : 0; arp_up_down_direction_is_up = false; } } else { temp_arp_index = temp_arp_index - 1; if (temp_arp_index < 0) { temp_arp_index = arp_num_held_notes > 1 ? 1 : 0; arp_up_down_direction_is_up = true; } } if (arp_num_held_notes > 0) base_note_for_arp_pattern = arp_held_notes[temp_arp_index]; else { return; } break;
                case ARP_RANDOM: if (arp_num_held_notes > 0) temp_arp_index = random(arp_num_held_notes); base_note_for_arp_pattern = arp_held_notes[temp_arp_index]; break;
                case ARP_ORDER_PLAYED: if (arp_num_played_order_notes > 0) { temp_played_order_index = (temp_played_order_index + 1) % arp_num_played_order_notes; base_note_for_arp_pattern = arp_notes_by_order[temp_played_order_index]; } else { return; } break;
                case ARP_CHORD: for(int i=0; i < arp_num_held_notes; ++i) { arpInternalNoteOn(arp_held_notes[i], arp_last_velocity); } arp_last_played_arp_note = arp_held_notes[0]; last_arp_step_time = millis(); return;
                case ARP_OCT_UP: case ARP_OCT_DOWN: temp_arp_index = (temp_arp_index + 1); if (temp_arp_index >= arp_num_held_notes) { temp_arp_index = 0; if (effective_mode == ARP_OCT_UP) { arp_octave_shift++; if (arp_octave_shift >= ARP_MAX_OCTAVE_STEPS) arp_octave_shift = 0; } else { arp_octave_shift--; if (arp_octave_shift <= -ARP_MAX_OCTAVE_STEPS) arp_octave_shift = 0; } } base_note_for_arp_pattern = arp_held_notes[temp_arp_index]; break;
            ...
            uint8_t final_arp_note = base_note_for_arp_pattern;

            if (current_arp_mode == ARP_OCT_UP || current_arp_mode == ARP_OCT_DOWN) {
                final_arp_note = base_note_for_arp_pattern + (arp_octave_shift * 12);
                if (final_arp_note > 127) final_arp_note = base_note_for_arp_pattern; 
                if (final_arp_note < 0) final_arp_note = base_note_for_arp_pattern;   
            }
            
            if (current_arp_mode != ARP_CHORD) {
                 arp_last_played_arp_note = final_arp_note;
                 arpInternalNoteOn(final_arp_note, arp_last_velocity);
            }

            if (play_tonic_separately) {
                uint8_t tonic_note = arp_held_notes[0]; 
                if (arp_last_played_tonic_note != tonic_note && arp_last_played_tonic_note != 0) {
                    arpInternalNoteOff(arp_last_played_tonic_note);
                    arp_last_played_tonic_note = 0; 
                }
                if (tonic_note != final_arp_note) { 
                     if(arp_last_played_tonic_note != tonic_note) { // Nur neu triggern, wenn sie nicht schon spielt
                        arpInternalNoteOn(tonic_note, arp_last_velocity > 30 ? arp_last_velocity - 30 : arp_last_velocity); 
                        arp_last_played_tonic_note = tonic_note;
                     }
                } else if (arp_last_played_tonic_note != 0 && tonic_note == final_arp_note) {
                    // Wenn Arp-Note = Tonika-Note, und Tonika spielte schon, dann muss sie nicht extra aus/an
                    // aber arp_last_played_tonic_note muss ggf. gelöscht werden, da die Arp-Note sie "übernimmt"
                    arp_last_played_tonic_note = 0; 
                }
            } else { 
                 if (arp_last_played_tonic_note != 0) {
