// ESP32-S3 - 6-VOICE POLYPHONIC SYNTH - ... (KORRIGIERT V27 - 10+ Waves, OLED Display)
...
#include <Adafruit_SSD1306.h> // Für OLED SSD1306

// -----------------------------------------------------------------------------
// KONSTANTEN & DEFINITIONEN
// -----------------------------------------------------------------------------
#define SAMPLE_RATE         22050
// ... (andere HW-Konstanten bleiben gleich) ...
...
#define ENCODER_SWITCH_PIN      GPIO_NUM_19 

// --- OLED Display ---
#define SCREEN_WIDTH 128 // OLED display width, in pixels
#define SCREEN_HEIGHT 64 // OLED display height, in pixels (oder 32 für kleinere Displays)
#define OLED_RESET    -1 // Reset pin # (or -1 if sharing Arduino reset pin)
#define OLED_I2C_ADDRESS 0x3C // Typische Adresse für SSD1306
...
// --- Wavetable Definitionen ---
#define WAVE_SINE           0
#define WAVE_SAW_DOWN       1 // Klassischer Sägezahn (abfallend)
#define WAVE_SQUARE         2 // 50% Puls
#define WAVE_TRIANGLE       3
#define WAVE_PULSE_25       4 // 25% Puls
#define WAVE_PULSE_12       5 // 12.5% Puls
#define WAVE_SYNC_SAW       6 // "Sync"-ähnlicher Klang
#define WAVE_FORMANT_A      7 // Formant "A"
#define WAVE_RAMP_UP        8 // Ansteigender Sägezahn
#define WAVE_HOLLOW_SQUARE  9 // Wenige ungerade Harmonische
#define WAVE_FORMANT_O     10 // Formant "O"
// #define WAVE_NOISE       11 // Wenn man Noise per Wavetable (kurz, periodisch) machen wollte
#define NUM_WAVETABLES     11 // << ERHÖHT >> 

enum EnvelopeStage { ENV_IDLE, ENV_ATTACK, ENV_DECAY, ENV_SUSTAIN, ENV_RELEASE };
struct VoiceState { /* ... unverändert ... */
...
volatile bool display_update_needed = true; // Flag für Display-Aktualisierung
...
unsigned long last_encoder_switch_press_time = 0;
const unsigned long ENCODER_SWITCH_DEBOUNCE_TIME = 70; 
...
void generateAllSynthWavetables() { 
    ...
    // WAVE_SQUARE (2)
    for (int k = 1; k <= 15; k += 2) { for (int i = 0; i < WAVETABLE_SIZE; ++i) { wavetables[WAVE_SQUARE][i] += (1.0f / k) * sinf(2.0f * PI * k * (float)i / WAVETABLE_SIZE); } } 
    normalizeWavetable(WAVE_SQUARE);
    // WAVE_TRIANGLE (3)
    ...
    // WAVE_HOLLOW_SQUARE (9) - 1st, 3rd, 5th harmonics
...
void processEncoderControls() { 
    if (digitalRead(ENCODER_SWITCH_PIN) == LOW) { 
        if (millis() - last_encoder_switch_press_time > ENCODER_SWITCH_DEBOUNCE_TIME) {
            if (xSemaphoreTake(voicesMutex, (TickType_t)10) == pdTRUE) {
                global_osc_wave_select_idx = (global_osc_wave_select_idx + 1) % NUM_WAVETABLES;
                display_update_needed = true; // Flag für Display setzen
                xSemaphoreGive(voicesMutex);
            }
            last_encoder_switch_press_time = millis();
        }
    }
}
...
void updateDisplay() {
    ...
    display.setTextSize(1); // Kleinere Schrift für mehr Text, oder 2 für größer
    ...
    if (global_osc_wave_select_idx < NUM_WAVETABLES) {
        ...
        display.println("Wave Err");
    }
    // Hier könnten weitere Infos angezeigt werden, z.B.
    // display.setCursor(0,10); display.print("Cut: "); display.print((int)smoothed_filter_cutoff_hz);
    // display.setCursor(0,20); display.print("Res: "); display.print(smoothed_filter_resonance, 1);
...
// -----------------------------------------------------------------------------
// MIDI CALLBACKS (unverändert)
// -----------------------------------------------------------------------------
void midiNoteOnHandler(byte ch, byte n, byte vel){ if(vel>0){ if(xSemaphoreTake(voicesMutex,(TickType_t)10)==pdTRUE){ int vi=findAvailableVoice(n);VoiceState&v=voices[vi];v.note=n;v.velocity_gain=(float)vel/127.0f;v.note_on_trigger=true;v.note_off_trigger=false;v.current_wave_idx=global_osc_wave_select_idx; v.phase_accumulator=0.0f; v.env_attack_rate=(global_attack_time_ms>0)?(1.0f/(global_attack_time_ms*0.001f*SAMPLE_RATE)):1.0f;v.env_decay_rate=(global_decay_time_ms>0)?(1.0f/(global_decay_time_ms*0.001f*SAMPLE_RATE)):1.0f;v.env_sustain_level_voice=global_sustain_level;v.env_release_rate=(global_release_time_ms>0)?(1.0f/(global_release_time_ms*0.001f*SAMPLE_RATE)):1.0f;xSemaphoreGive(voicesMutex);}}else{midiNoteOffHandler(ch,n,vel);}}
void midiNoteOffHandler(byte ch, byte n, byte vel){ if(xSemaphoreTake(voicesMutex,(TickType_t)10)==pdTRUE){ for(int i=0;i<NUM_VOICES_ACTIVE;++i)if(voices[i].is_playing&&voices[i].note==n)voices[i].note_off_trigger=true;xSemaphoreGive(voicesMutex);}}

// -----------------------------------------------------------------------------
// AUDIO PROCESSING (SVF unverändert, fillAudioBuffer angepasst)
// -----------------------------------------------------------------------------
inline float processGlobalSVF(float input_sample) { /* ... unverändert ... */ 
    ...
    float resonance_q = smoothed_filter_resonance; 
    if (resonance_q < 0.5f) resonance_q = 0.5f; 
    if (resonance_q > 6.0f) resonance_q = 6.0f; 
    ...
    global_filter_bp_output = current_bp_output; 
    global_filter_lp_output = current_lp_output; 
...
void fillAudioBuffer() { /* ... (Pitchbend angewendet) ... */
  ...
  while (true) {
    ...
    if (current_time_ms - last_pot_read_control_task >= DIRECT_POT_READ_INTERVAL_MS) {
      ...
      if (current_time_ms - last_debug_print_time > 250) { 
          Serial.print("PB: "); Serial.print(global_pitchbend_factor, 3); 
          Serial.print(" (Raw:"); Serial.print(raw_pot_pitchbend); Serial.print(") | ");
          Serial.print("LFO Rate Tgt: "); Serial.print(global_lfo_rate_hz_target, 2);
          Serial.print(", Sm: "); Serial.print(smoothed_lfo_rate_hz, 2);
          Serial.print(" | PDBase: "); Serial.print(global_lfo_pitch_depth_base_poti, 2);
          Serial.print(", ModWh: "); Serial.print(global_mod_wheel_value, 2); 
          Serial.print(", PDEff: "); Serial.print(global_lfo_pitch_depth, 2);
          Serial.print(" | FDep: "); Serial.print(global_lfo_filter_depth, 2);
          Serial.print(" | Inc: "); Serial.println(lfo_increment, 4);
          last_debug_print_time = current_time_ms;
      }
    }

    // Display seltener aktualisieren, um I2C Bus nicht zu überlasten
    if (display_update_needed && (current_time_ms - last_display_update_time > 50)) { // z.B. max alle 50ms
        updateDisplay();
...
void setup() {
  ...
  Serial.println("\nESP32-S3 Poly Synth - (V27 - 10+ Waves, OLED)");

  // I2C und Display initialisieren
  Wire.begin(); // Standard I2C Pins für ESP32-S3 (SDA=21, SCL=22, kann variieren)
                // Wenn du andere Pins brauchst: Wire.begin(SDA_PIN, SCL_PIN);
  if(!display.begin(SSD1306_SWITCHCAPVCC, OLED_I2C_ADDRESS)) {
    Serial.println(F("SSD1306 allocation failed"));
    // Hier könntest du in eine Endlosschleife gehen oder ohne Display weitermachen
  } else {
    ...
    display.setTextColor(SSD1306_WHITE);
    display.setCursor(0,0);
    display.println("Synth Ready!");
    display.setCursor(0,10);
    display.print("Wave: ");
    display.print(wavetable_names[global_osc_wave_select_idx]);
    display.display();
    display_update_needed = false; // Initialanzeige gemacht
  }
  ...
  Serial.println("Poti & Encoder Pins initialized.");
  ...
  xTaskCreatePinnedToCore( controlMainTask, "ControlMain", 4096, NULL, 5, &controlMainTaskHandle, 0 ); // Stack für Control Task ggf. erhöhen, falls Display viel braucht
