// ESP32-S3 - 6-VOICE POLYPHONIC SYNTH - (V33.4 - Volatile Fix, Arp Modus "Off" entfernt)
...
#define NUM_VOICES_ACTIVE   2  // << WIEDER AUF 6 GESETZT, WAR VORHER 2 >>
...
// --- Arpeggiator ---
// ARP_OFF Modus entfernt aus der Auswahl, wird nur noch über arp_is_on gesteuert
enum ArpMode { ARP_UP, ARP_DOWN, ARP_UP_DOWN, ARP_RANDOM, NUM_ARP_MODES_SELECTABLE }; // NUM_ARP_MODES_SELECTABLE ist jetzt 4
const char* arp_mode_names[NUM_ARP_MODES_SELECTABLE] = {"Up", "Down", "Up/Down", "Random"}; // Namen für die auswählbaren Modi
...
void processArpControls() {
    ...
    if (digitalRead(ARP_MODE_SWITCH_PIN) == LOW) {
        if (millis() - last_arp_mode_press_time > ARP_MODE_DEBOUNCE_TIME) {
            current_arp_mode = (ArpMode)((current_arp_mode + 1) % NUM_ARP_MODES_SELECTABLE); // Geändert zu NUM_ARP_MODES_SELECTABLE
            // if (current_arp_mode == ARP_OFF) arp_is_on = false; // Nicht mehr nötig, da ARP_OFF nicht mehr wählbar
...
void handleNoteOff(byte channel, byte note, byte velocity) {
    ...
    if (found_and_removed) {
        if (!arp_is_on) { arpInternalNoteOff(note); } 
        else { if (arp_last_played_note == note) { arpInternalNoteOff(arp_last_played_note); arp_last_played_note = 0; }
            ...
            else { if (removed_idx != -1 && removed_idx < arp_current_note_index) { arp_current_note_index = arp_current_note_index - 1; } if (arp_current_note_index >= arp_num_held_notes && arp_num_held_notes > 0) { arp_current_note_index = 0; } }
        }
    }
}
...
void processArpeggiator() {
    ...
    if (millis() - last_arp_step_time >= ARP_STEP_INTERVAL_MS) {
        ...
        if (arp_num_held_notes > 0) { 
            switch (current_arp_mode) { // Verwendet NUM_ARP_MODES_SELECTABLE implizit durch den Modulo-Operator in processArpControls
                ...
                // ARP_OFF wird hier nicht mehr benötigt, da arp_is_on den Arp steuert
            }
            if (arp_num_held_notes > 0) { // Erneute Prüfung nach Moduslogik
  ...
  while (true) {
    ...
    if (current_time_ms - last_pot_read_control_task >= DIRECT_POT_READ_INTERVAL_MS) {
      ...
      if (current_time_ms - last_debug_print_time > 250) { 
          ...
          Serial.print(" | Notes: "); Serial.print(arp_num_held_notes);
          Serial.print(" | PB: "); Serial.print(global_pitchbend_factor, 2); 
          Serial.println();
...
void setup() {
  Serial.begin(115200); 
  Serial.println("\nESP32-S3 Poly Synth - (V33.4 - Volatile Fix, Arp Modus 'Off' entfernt)"); 
  ...
  if(!display.begin(SSD1306_SWITCHCAPVCC, OLED_I2C_ADDRESS)) {
    ...
    display.setCursor(0,0); display.println("Synth V33.4 ARP"); 
    display_update_needed = true; 
  }
