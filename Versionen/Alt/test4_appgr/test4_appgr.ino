// ESP32-S3 - 6-VOICE POLYPHONIC SYNTH - (V33.3 - Arpeggiator TEMPORÄR KOMPLETT DEAKTIVIERT)
...
// #define ARP_ONOFF_SWITCH_PIN GPIO_NUM_18 // Deaktiviert
// #define ARP_MODE_SWITCH_PIN  GPIO_NUM_17 // Deaktiviert
...
// --- Arpeggiator (Variablen bleiben, werden aber nicht genutzt) ---
...
volatile float global_attack_time_ms = 50.0f; // Zurück zu Standardwerten, falls Arp-Werte störten
...
// Arp Debounce Zeiten nicht mehr relevant
// const unsigned long ARP_ONOFF_DEBOUNCE_TIME = 150; 
...
void readAllDirectPotis() { /* ... (unverändert von V27) ... */
  ...
  if (raw_pot_master_volume < MASTER_VOLUME_POTI_DEADZONE) { 
      ...
      if(global_master_volume > 1.0f) global_master_volume = 1.0f; 
  }
}
...
// void processArpControls() { // << VORERST DEAKTIVIERT >>
// }
...
void updateDisplay() {
    ...
    // display.setCursor(0,10); // << Platz für Arp-Info, wenn wieder aktiv >>
    // display.print("Arp: ");
...
// -----------------------------------------------------------------------------
// MIDI CALLBACKS (Original-Handler ohne Arp-Logik)
// -----------------------------------------------------------------------------
void handleNoteOn(byte channel, byte note, byte velocity) {
    if (xSemaphoreTake(voicesMutex, (TickType_t)10) == pdTRUE) {
        ...
        v.note_on_trigger = true;
        v.note_off_trigger = false;
...
void controlMainTask(void *pvParameters) {
  ...
  while (true) {
    ...
    // processArpControls(); // << ARP TASTER DEAKTIVIERT >>
    // processArpeggiator(); // << ARP LOGIK DEAKTIVIERT >>
    ...
    if (current_time_ms - last_pot_read_control_task >= DIRECT_POT_READ_INTERVAL_MS) {
      ...
      if (current_time_ms - last_debug_print_time > 250) { 
          ...
          // Serial.print(" | Arp: "); Serial.print(arp_is_on ? arp_mode_names[current_arp_mode] : "Off"); // Arp-Debug entfernt
          // Serial.print(" | Notes: "); Serial.print(arp_num_held_notes);
          // if(arp_is_on) Serial.print("*");
          Serial.println();
...
void setup() {
  Serial.begin(115200); 
  Serial.println("\nESP32-S3 Poly Synth - (V33.3 - Arpeggiator komplett deaktiviert)"); 
  ...
  if(!display.begin(SSD1306_SWITCHCAPVCC, OLED_I2C_ADDRESS)) {
    ...
    display.setCursor(0,0); display.println("Synth V33.3"); 
    display_update_needed = true; 
  }

  pinMode(WAVE_SWITCH_PIN, INPUT_PULLUP); 
  // pinMode(ARP_ONOFF_SWITCH_PIN, INPUT_PULLUP); // Deaktiviert
  // pinMode(ARP_MODE_SWITCH_PIN, INPUT_PULLUP);  // Deaktiviert
  ...
  xTaskCreatePinnedToCore( controlMainTask, "ControlMain", 4096, NULL, 5, &controlMainTaskHandle, 0 ); // Stack von 5120 auf 4096 zurück
