// ESP32-S3 - 6-VOICE POLYPHONIC SYNTH - (V33.5 - Chord Reliability Fix)
...
#define WAVETABLE_SIZE      256 // !!! WICHTIG: DIESE DEFINITION MUSS HIER STEHEN !!!
...
// --- Arpeggiator ---
enum ArpMode { ARP_UP, ARP_DOWN, ARP_UP_DOWN, ARP_RANDOM, NUM_ARP_MODES_SELECTABLE };
const char* arp_mode_names[NUM_ARP_MODES_SELECTABLE] = {"Up", "Down", "Up/Down", "Random"};
...
float wavetables[NUM_WAVETABLES][WAVETABLE_SIZE]; // WAVETABLE_SIZE wird hier benötigt
...
const unsigned long ARP_MODE_DEBOUNCE_TIME = 150;
...
// WAVETABLE_SIZE wird hier im Parameter und im Funktionskörper benötigt
...
// WAVETABLE_SIZE und wavetables[] werden hier benötigt
...
// WAVETABLE_SIZE und wavetables[] werden hier intensiv genutzt
void generateAllSynthWavetables() {
    for (int w = 0; w < NUM_WAVETABLES; ++w) { for (int i = 0; i < WAVETABLE_SIZE; ++i) { wavetables[w][i] = 0.0f; } }
    for (int i = 0; i < WAVETABLE_SIZE; ++i) { wavetables[WAVE_SINE][i] = sinf(2.0f * PI * (float)i / WAVETABLE_SIZE); }
    for (int i = 0; i < WAVETABLE_SIZE; ++i) { wavetables[WAVE_SAW_DOWN][i] = 1.0f - 2.0f * (float)i / WAVETABLE_SIZE; }
    for (int k = 1; k <= 15; k += 2) { for (int i = 0; i < WAVETABLE_SIZE; ++i) { wavetables[WAVE_SQUARE][i] += (1.0f / k) * sinf(2.0f * PI * k * (float)i / WAVETABLE_SIZE); } } normalizeWavetable(WAVE_SQUARE);
    for (int i = 0; i < WAVETABLE_SIZE; ++i) { float val = 2.0f * (float)i / WAVETABLE_SIZE; if (val > 1.0f) { val = 2.0f - val; } wavetables[WAVE_TRIANGLE][i] = 2.0f * val - 1.0f; }
...
int findAvailableVoice(uint8_t noteVal) {
    for (int i = 0; i < NUM_VOICES_ACTIVE; ++i) { if (voices[i].is_playing && voices[i].note == noteVal) { return i; } }
    for (int i = 0; i < NUM_VOICES_ACTIVE; ++i) { if (!voices[i].is_playing && voices[i].env_stage == ENV_IDLE) { return i; } }
    int best_candidate_release = -1; float min_level_release = 2.0f;
    for (int i = 0; i < NUM_VOICES_ACTIVE; i++) { if (voices[i].is_playing && voices[i].env_stage == ENV_RELEASE && voices[i].env_level < min_level_release) { min_level_release = voices[i].env_level; best_candidate_release = i; } }
    if (best_candidate_release != -1) { return best_candidate_release; }
    float min_level_overall = 2.0f; int best_candidate_overall = 0;
    for (int i = 0; i < NUM_VOICES_ACTIVE; i++) { if (voices[i].env_stage != ENV_ATTACK) { if (voices[i].env_level < min_level_overall) { min_level_overall = voices[i].env_level; best_candidate_overall = i; } } }
...
void processArpControls() {
    if (digitalRead(ARP_ONOFF_SWITCH_PIN) == LOW) {
        if (millis() - last_arp_onoff_press_time > ARP_ONOFF_DEBOUNCE_TIME) {
            arp_is_on = !arp_is_on;
            if (!arp_is_on && arp_last_played_note != 0) {
...
void handleNoteOn(byte channel, byte note, byte velocity) {
    ...
    for (int i = 0; i < arp_num_held_notes; ++i) { if (arp_held_notes[i] == note) { note_already_held = true; break; } }
    if (!note_already_held && arp_num_held_notes < ARP_MAX_NOTES) {
        arp_held_notes[arp_num_held_notes] = note; arp_num_held_notes = arp_num_held_notes + 1;
        for (int i = 0; i < arp_num_held_notes - 1; i++) { for (int j = i + 1; j < arp_num_held_notes; j++) { if (arp_held_notes[i] > arp_held_notes[j]) { uint8_t temp = arp_held_notes[i]; arp_held_notes[i] = arp_held_notes[j]; arp_held_notes[j] = temp; } } }
    }
    arp_last_velocity = velocity;
    if (!arp_is_on) { arpInternalNoteOn(note, velocity); }
    else { if (arp_num_held_notes == 1) { arp_current_note_index = 0; } }
}
void handleNoteOff(byte channel, byte note, byte velocity) {
    bool found_and_removed = false; int removed_idx = -1;
    for (int i = 0; i < arp_num_held_notes; ++i) { if (arp_held_notes[i] == note) { found_and_removed = true; removed_idx = i; for (int j = i; j < arp_num_held_notes - 1; ++j) { arp_held_notes[j] = arp_held_notes[j+1]; } arp_num_held_notes = arp_num_held_notes - 1; break; } }
    if (found_and_removed) {
        if (!arp_is_on) { arpInternalNoteOff(note); }
        else {
            if (arp_last_played_note == note) {
                arpInternalNoteOff(arp_last_played_note);
                arp_last_played_note = 0;
            }
            if (arp_num_held_notes == 0) { if (arp_last_played_note != 0) arpInternalNoteOff(arp_last_played_note); arp_last_played_note = 0; arp_current_note_index = 0; }
            else { if (removed_idx != -1 && removed_idx < arp_current_note_index) { arp_current_note_index = arp_current_note_index - 1; } if (arp_current_note_index >= arp_num_held_notes && arp_num_held_notes > 0) { arp_current_note_index = 0; } if (arp_current_note_index < 0 ) arp_current_note_index = 0;}
        }
    }
}

void processArpeggiator() {
    if (!arp_is_on || arp_num_held_notes == 0) { if (arp_last_played_note != 0) { arpInternalNoteOff(arp_last_played_note); arp_last_played_note = 0; } return; }
    if (millis() - last_arp_step_time >= ARP_STEP_INTERVAL_MS) {
        ...
        if (arp_num_held_notes > 0) {
            switch (current_arp_mode) {
                case ARP_UP: arp_current_note_index = (arp_current_note_index + 1) % arp_num_held_notes; break;
                case ARP_DOWN: arp_current_note_index = arp_current_note_index - 1; if (arp_current_note_index < 0) { arp_current_note_index = arp_num_held_notes - 1; } break;
                case ARP_UP_DOWN: if (arp_up_down_direction_is_up) { arp_current_note_index = arp_current_note_index + 1; if (arp_current_note_index >= arp_num_held_notes) { arp_current_note_index = arp_num_held_notes > 1 ? arp_num_held_notes - 2 : 0; arp_up_down_direction_is_up = false; } } else { arp_current_note_index = arp_current_note_index - 1; if (arp_current_note_index < 0) { arp_current_note_index = arp_num_held_notes > 1 ? 1 : 0; arp_up_down_direction_is_up = true; } } break;
                case ARP_RANDOM: if (arp_num_held_notes > 0) { arp_current_note_index = random(arp_num_held_notes); } break;
            }
            if (arp_num_held_notes > 0) {
                 if (arp_current_note_index < 0) arp_current_note_index = 0;
                 if (arp_current_note_index >= arp_num_held_notes) arp_current_note_index = arp_num_held_notes -1;
            }
            arp_last_played_note = arp_held_notes[arp_current_note_index];
            arpInternalNoteOn(arp_last_played_note, arp_last_velocity);
        } else { arp_last_played_note = 0; }
...
// AUDIO PROCESSING
// -----------------------------------------------------------------------------
// wavetables[] wird hier benötigt
inline float processGlobalSVF(float input_sample) {
    float cutoff_freq_hz = smoothed_filter_cutoff_hz;
    if (lfo_increment > 0.000001f && global_lfo_filter_depth > 0.001f) {
        float lfo_val_for_filter = interpolateWavetable(wavetables[0], lfo_phase); // Using sine for LFO
        float mod_range = cutoff_freq_hz * 0.8f;
        cutoff_freq_hz += lfo_val_for_filter * global_lfo_filter_depth * mod_range;
        if (cutoff_freq_hz < 20.0f) cutoff_freq_hz = 20.0f;
        if (cutoff_freq_hz > (float)SAMPLE_RATE / 2.1f) cutoff_freq_hz = (float)SAMPLE_RATE / 2.1f;
    }
    float resonance_q = smoothed_filter_resonance; if (resonance_q < 0.5f) resonance_q = 0.5f; if (resonance_q > 6.0f) resonance_q = 6.0f;
    float g = tanf(PI * cutoff_freq_hz / SAMPLE_RATE);
    float k = 1.0f / resonance_q;
    float hp_output = input_sample - global_filter_integrator2_state - k * global_filter_integrator1_state;
    float current_bp_output = g * hp_output + global_filter_integrator1_state;
    float current_lp_output = g * current_bp_output + global_filter_integrator2_state;
    global_filter_integrator1_state = current_bp_output;
    global_filter_integrator2_state = current_lp_output;
    global_filter_bp_output = current_bp_output; global_filter_lp_output = current_lp_output;
    if (isnan(global_filter_integrator1_state) || isinf(global_filter_integrator1_state) || isnan(global_filter_integrator2_state) || isinf(global_filter_integrator2_state)) {
        global_filter_integrator1_state = 0.0f; global_filter_integrator2_state = 0.0f;
        global_filter_bp_output = 0.0f; global_filter_lp_output = 0.0f;
        return 0.0f;
    }
    return global_filter_lp_output;
}
...
void fillAudioBuffer() {
  ...
  float lfo_val_for_pitch = interpolateWavetable(wavetables[0], lfo_phase); // Using sine for LFO
  ...
  for (int frame = 0; frame < DMA_BUFFER_FRAMES; ++frame) {
    ...
    for (int v_idx = 0; v_idx < NUM_VOICES_ACTIVE; ++v_idx) {
      ...
      if (v_local.is_playing && (v_local.env_level > 0.0001f || v_local.env_stage == ENV_ATTACK)) {
        ...
        v_local.phase_increment = (current_note_freq * WAVETABLE_SIZE) / SAMPLE_RATE; // WAVETABLE_SIZE
        v_local.phase_accumulator += v_local.phase_increment;
        if (v_local.phase_accumulator >= WAVETABLE_SIZE) { v_local.phase_accumulator -= WAVETABLE_SIZE; } // WAVETABLE_SIZE
        float main_osc_output = interpolateWavetable(wavetables[v_local.current_wave_idx], v_local.phase_accumulator); // wavetables[]
...
void setup() {
  Serial.begin(115200);
  Serial.println("\nESP32-S3 Poly Synth - (V33.5 - Chord Reliability Fix)");
  ...
  if(!display.begin(SSD1306_SWITCHCAPVCC, OLED_I2C_ADDRESS)) {
    ...
    display.setCursor(0,0); display.println("Synth V33.5 CHORD");
    display.display();
    display_update_needed = true;
  }
  ...
  generateAllSynthWavetables(); Serial.println("Wavetables generated."); // Benötigt WAVETABLE_SIZE und wavetables[]
  ...
  xTaskCreatePinnedToCore( controlMainTask, "ControlMain", 4096 + 1024, NULL, 5, &controlMainTaskHandle, 0 );
