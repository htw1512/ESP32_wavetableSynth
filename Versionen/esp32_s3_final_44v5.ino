// ESP32-S3 - Polyphoner Synth - (V34.5.5 - Cleaned Wavetable Count)
...
// Wellenform-Indizes (optional, aber hilfreich für Lesbarkeit)
#define WAVE_SINE           0
...
// ... (Indizes für neue Wellenformen könnten hier weitergeführt werden, wenn man möchte) ...

// Anzahl der *tatsächlich implementierten* Wellenformen
#define NUM_WAVETABLES_TOTAL 51 // 11 Original + 30 "Interessante" + 10 Klavier/Synth
...
void generateAllSynthWavetables() { 
    for (int w = 0; w < NUM_WAVETABLES_TOTAL; ++w) {
       ...
       // wavetable_display_names[w] = "Not Set"; // Initialer Platzhalter
    }
    ...
    // Zusätzliche 30 "interessante" Wellenformen (Index 11-40)
    ...
    // 10 neue Klavier/Synth-ähnliche Wellenformen (beginnend bei Index 41)
    if (idx < NUM_WAVETABLES_TOTAL) { // Klavierartig (Struck String mit mehr Obertönen)
        for (int i = 0; i < WAVETABLE_SIZE; ++i) {
            ...
            val += 0.1f  * sinf(2 * PI * t_norm * 6.0f) * expf(-t_norm * 11.0f);
            val += 0.05f * sinf(2 * PI * t_norm * 7.0f) * expf(-t_norm * 12.0f);
    ...
    if (idx < NUM_WAVETABLES_TOTAL) { // Rhodes-artiges E-Piano (weicher, glockiger)
        for (int i = 0; i < WAVETABLE_SIZE; ++i) {
            ...
            val += 0.1f  * sinf(2*PI*t_norm * 3.5f) * expf(-t_norm * 5.0f); 
            val += 0.3f  * sinf(2*PI*t_norm * 6.0f) * expf(-t_norm * 6.0f);
            wavetables[idx][i] = val;
        }
        normalizeWavetable(idx); wavetable_display_names[idx++] = "Rhodesy EP";
    }
    if (idx < NUM_WAVETABLES_TOTAL) { // Einfache Kick Drum (Pitch Envelope im Wavetable)
        for (int i = 0; i < WAVETABLE_SIZE; ++i) {
            float t_norm = (float)i / WAVETABLE_SIZE; 
            float freq_mult = 4.0f * expf(-t_norm * 15.0f) + 1.0f; 
            float amp_decay = expf(-t_norm * 8.0f);
            wavetables[idx][i] = sinf(2 * PI * t_norm * freq_mult) * amp_decay;
        }
        normalizeWavetable(idx); wavetable_display_names[idx++] = "Kick WT";
    }
     if (idx < NUM_WAVETABLES_TOTAL) { // Einfache Snare/Hihat (gefiltertes Rauschen am Anfang)
        for (int i = 0; i < WAVETABLE_SIZE; ++i) {
            float t_norm = (float)i / WAVETABLE_SIZE;
            float noise = ((float)rand() / RAND_MAX) * 2.0f - 1.0f;
            float body = sinf(2 * PI * t_norm * 5.0f); 
            wavetables[idx][i] = (0.7f * noise * expf(-t_norm * 30.0f)) + (0.3f * body * expf(-t_norm * 10.0f));
        }
        normalizeWavetable(idx); wavetable_display_names[idx++] = "Snareish WT";
    }
    if (idx < NUM_WAVETABLES_TOTAL) { // Moderner "Wobble" Bass (Sinus mit langsamer FM im Table)
        float fm_rate = 0.25f; 
        float fm_depth_start = 0.0f;
        float fm_depth_end = 2.5f; 
        for (int i = 0; i < WAVETABLE_SIZE; ++i) {
            float t_norm = (float)i / WAVETABLE_SIZE;
            float current_fm_depth = fm_depth_start + (fm_depth_end - fm_depth_start) * t_norm;
            wavetables[idx][i] = sinf(2 * PI * t_norm + current_fm_depth * sinf(2 * PI * t_norm * fm_rate));
        }
        normalizeWavetable(idx); wavetable_display_names[idx++] = "Wobble Bass";
    }
    if (idx < NUM_WAVETABLES_TOTAL) { // "Electric" - Rechteck mit einigen ungeraden Harmonischen, die ein/ausblenden
        for (int i = 0; i < WAVETABLE_SIZE; ++i) {
            float t_norm = (float)i / WAVETABLE_SIZE;
            float phase = 2 * PI * t_norm;
            wavetables[idx][i] = 0.6f * ((fmod(phase, 2*PI) < PI) ? 1.0f : -1.0f); 
            wavetables[idx][i] += 0.3f * sinf(3 * phase) * sinf(0.5f * PI * t_norm); 
            wavetables[idx][i] += 0.1f * sinf(5 * phase) * cosf(0.5f * PI * t_norm); 
        }
        normalizeWavetable(idx); wavetable_display_names[idx++] = "EvolvingRect";
    }
    if (idx < NUM_WAVETABLES_TOTAL) { // "Resonant Pad" - mehrere Saws mit leicht unterschiedlichen Phasen und Amplituden
        for (int i = 0; i < WAVETABLE_SIZE; ++i) {
            float p = (float)i / WAVETABLE_SIZE;
            wavetables[idx][i]  = 0.4f * (1.0f - 2.0f * fmod(p + 0.00f, 1.0f));
            wavetables[idx][i] += 0.3f * (1.0f - 2.0f * fmod(p + 0.05f, 1.0f));
            wavetables[idx][i] += 0.2f * (1.0f - 2.0f * fmod(p + 0.11f, 1.0f));
            wavetables[idx][i] += 0.1f * (1.0f - 2.0f * fmod(p - 0.07f, 1.0f));
        }
        normalizeWavetable(idx); wavetable_display_names[idx++] = "Reso Pad";
    }
     if (idx < NUM_WAVETABLES_TOTAL) { // "Space Voice" - Sinus + hohe, leicht verstimmte Harmonische
        for (int i = 0; i < WAVETABLE_SIZE; ++i) {
            float t = 2.0f * PI * (float)i / WAVETABLE_SIZE;
            wavetables[idx][i] = 0.7f * sinf(t) +
                                                  0.2f * sinf(t * 7.05f) + 
                                                  0.1f * sinf(t * 11.95f);
        }
        normalizeWavetable(idx); wavetable_display_names[idx++] = "Space Voice";
    }
    if (idx < NUM_WAVETABLES_TOTAL) { // "8-Bit Saw" - Treppensägezahn
        int steps = 16;
        for (int i = 0; i < WAVETABLE_SIZE; ++i) {
            float saw_val = -1.0f + 2.0f * (float)i / WAVETABLE_SIZE; 
            float quantized_val = floorf((saw_val + 1.0f) * 0.5f * steps);
            wavetables[idx][i] = (quantized_val / (steps -1) ) * 2.0f - 1.0f;
        }
        normalizeWavetable(idx); wavetable_display_names[idx++] = "8-Bit Saw";
    }
    if (idx < NUM_WAVETABLES_TOTAL) { // "FM Clav" - Einfache FM mit perkussivem Charakter im Table
        float mod_freq_ratio = 1.414f; 
        float mod_depth_start = 1.5f;
        float mod_depth_end = 0.1f;
        for (int i = 0; i < WAVETABLE_SIZE; ++i) {
            float t_norm = (float)i / WAVETABLE_SIZE;
            float t_angle = 2.0f * PI * t_norm;
            float current_mod_depth = mod_depth_start * expf(-t_norm * 8.0f) + mod_depth_end; 
            wavetables[idx][i] = sinf(t_angle + current_mod_depth * sinf(mod_freq_ratio * t_angle)) * expf(-t_norm * 6.0f);
        }
        normalizeWavetable(idx); wavetable_display_names[idx++] = "FM Clav";
    }
    
    Serial.printf("Total %d of %d wavetables generated.\n", idx, NUM_WAVETABLES_TOTAL);
    // Sicherstellen, dass alle Namen gesetzt sind, falls idx < NUM_WAVETABLES_TOTAL
    for (int i = idx; i < NUM_WAVETABLES_TOTAL; ++i) { 
        if(wavetable_display_names[i] == nullptr || strcmp(wavetable_display_names[i], "Unassigned") == 0 || strcmp(wavetable_display_names[i], "N/A") == 0 ) {
             wavetable_display_names[i] = "Not Impl."; // Klarer Hinweis, falls ein Slot nicht gefüllt wurde
        }
    }
}

// Voice Allocation Logik (exakt wie in V34.1)
...
void processArpControls() { 
        ...
        if (millis() - last_arp_onoff_press_time > ARP_ONOFF_DEBOUNCE_TIME) { 
            ...
            if (!arp_is_on) { 
                if (arp_last_played_arp_note != 0) { 
                    ...
                    if(is_oct_mode && arp_octave_shift != 0) note_to_stop += arp_octave_shift * 12;
    ...
    if (digitalRead(ARP_MODE_SWITCH_PIN) == LOW) { 
        if (millis() - last_arp_mode_press_time > ARP_MODE_DEBOUNCE_TIME) { 
            // Vor dem Moduswechsel laufende Arp-Noten stoppen
            if (arp_is_on) {
                if (arp_last_played_arp_note != 0) {
                    uint8_t note_to_stop = arp_last_played_arp_note;
                    bool is_oct_mode_old = (current_arp_mode == ARP_OCT_UP || current_arp_mode == ARP_OCT_DOWN);
...
void updateDisplay() {
    ...
    char buffer[32]; // Buffer für sprintf
    ...
    if (arp_is_on) {
        ...
        if (current_arp_mode == ARP_OCT_UP || current_arp_mode == ARP_OCT_DOWN) {
            display.print(" O:"); display.print(arp_octave_shift);
        }
    } else { display.print("Arp: Off"); }
    ...
    // Zeile 4: LFO & Master Volume
    display.setCursor(0, 30);
    snprintf(buffer, sizeof(buffer), "LFO:%.1fHz Vol:%.0f%%", smoothed_lfo_rate_hz, global_master_volume * 100);
    display.print(buffer);

    // Zeile 5: Mod Wheel & Pitch Bend
    display.setCursor(0, 40); 
    float pb_display = (global_pitchbend_factor - 1.0f) * 100.0f; // Als +/- Prozent Abweichung von 1.0
    snprintf(buffer, sizeof(buffer), "MW:%.0f%% PB:%+.0f%%", global_mod_wheel_value * 100, pb_display );
    display.print(buffer);
...
void handleNoteOff(byte channel, byte note, byte velocity) {
    ...
    if (found_and_removed_sorted || found_and_removed_order) { 
        if (!arp_is_on) {
            ...
            if (is_octave_mode && arp_last_played_arp_note != 0) { 
    ...
    if (millis() - last_arp_step_time >= current_arp_interval_ms) {
        ...
        if (arp_num_held_notes > 0) {
            ...
            if (play_tonic_separately) {
                ...
                } else if (arp_last_played_tonic_note != 0 && tonic_note == final_arp_note_to_play) {
                    arp_last_played_tonic_note = 0; 
                }
            } else { 
                 if (arp_last_played_tonic_note != 0) {
